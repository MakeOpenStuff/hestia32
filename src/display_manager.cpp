...
void display_create_ui(bool skip_calibration, bool provisioning_mode)
{
    ...
    // If in provisioning mode, show minimal UI with just text
    if (provisioning_mode) {
        ESP_LOGI(TAG, "Showing minimal provisioning UI");
        ...
        return;  // Skip creating heavy UI
    }

    // Create full main UI only if not in provisioning mode
    ...
    lv_timer_t *tmr = lv_timer_create([](lv_timer_t *t){
        ...
        if (now - last_t >= 1000) {
            char buf[32];
            lv_snprintf(buf, sizeof(buf), "FPS:%lu", (unsigned long)s_flush_count);
    ...
    // Touch dot to visualize touch - create LAST so it's on top
