...
/**
 * Background task to start WiFi/protocol connection without blocking main task
 */
static void protocol_start_task(void* arg)
{
	ESP_LOGI(TAG, "Protocol start task running...");
	esp_err_t ret = protocol_manager_start();
	if (ret == ESP_OK) {
		ESP_LOGI(TAG, "Protocol started successfully");
	} else {
		ESP_LOGE(TAG, "Failed to start protocol");
	}
	vTaskDelete(NULL);  // Task exits after protocol starts
}
...
/* Step 2: Check WiFi provisioning BEFORE initializing main UI */
...
/* Check if onboarding is needed BEFORE creating main UI */
...
display_clear_screen();  // Clear onboarding UI
}
...
ESP_LOGI(TAG, "User skipped WiFi provisioning - continuing without WiFi");
		...
		/* Clear provisioning UI before creating main UI */
display_clear_screen();
}
...
ESP_LOGI(TAG, "Provisioning started - running headless (display disabled)");
while (1) {
vTaskDelay(pdMS_TO_TICKS(100));
}
}
...
// Initialize sensors BEFORE starting WiFi to prevent blocking
	// Initialize relay manager and sensors BEFORE starting WiFi to prevent blocking		ESP_LOGI(TAG, "Initializing relay manager...");
		ret = relay_manager_init();
		if (ret == ESP_OK) {
			ESP_LOGI(TAG, "Relay manager initialized successfully");
		} else {
			ESP_LOGE(TAG, "Relay manager initialization failed: %s", esp_err_to_name(ret));
		}

		ESP_LOGI(TAG, "Initializing SHT45 sensor...");
		user_settings_init();

		/* Log current temperature unit setting */
		temp_unit_t boot_temp_unit = user_settings_get_temp_unit();
		ESP_LOGI(TAG, "Temperature unit at boot: %s",
		         boot_temp_unit == TEMP_UNIT_FAHRENHEIT ? "Fahrenheit" : "Celsius");
		...
		if (sensor_available) {
			...
			ESP_LOGW(TAG, "SHT45 sensor initialization failed: %s (continuing anyway)", esp_err_to_name(ret));
		}
		ESP_LOGI(TAG, "Sensor availability status: %s", sensor_available ? "AVAILABLE" : "NOT AVAILABLE");

		// Start WiFi/protocol connection in background task to avoid blocking sensor readings
		ESP_LOGI(TAG, "Starting protocol connection in background...");
		xTaskCreate(
			protocol_start_task,
			"protocol_start",
			4096,
		...
		ESP_LOGI(TAG, "Main initialization complete. Sensor loop starting.");
		...
		if (!first_read_done || (now - last_sensor_read >= 5)) {
			if (sensor_available) {
				...
				if (read_ret == ESP_OK) {
					...
					ESP_LOGW(TAG, "Sensor read failed: %s", esp_err_to_name(read_ret));
				}
			} else {
				if (!first_read_done) {
					ESP_LOGW(TAG, "Skipping sensor read - sensor not available");
					first_read_done = true;  // Don't spam this warning
				}
			}
